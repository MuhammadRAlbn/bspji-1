    ...
    closeLightbox() {
        ...
        <main
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-1">
                ...
                <button type="button" @click="tab = 'spm'"
                    ...
                    class="group flex scale-100 items-center gap-[15px] rounded-[12px] border px-5 py-4 text-left transition-all duration-300 ease-in-out hover:scale-[1.02]">
                    <svg class="h-5 w-5 shrink-0" :class="tab === 'spm' ? 'text-white' : 'text-slate-500'" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <article class="min-h-[85vh] pb-32 sm:pb-[450px]">
                ...
                {{-- Tab SPM --}}
                <section x-show="tab === 'spm'" x-cloak x-transition:enter="transition ease-out duration-400"
                    x-transition:enter-start="opacity-0 translate-y-4"
                    x-transition:enter-end="opacity-100 translate-y-0">
                    <div class="flex flex-col items-start gap-12">
                        @if($spmLsih && $spmLsih->image_path)
                            <button type="button"
                                @click="openLightbox('{{ asset('storage/' . $spmLsih->image_path) }}', 'Standar Pelayanan Minimal (SPM) Lembaga Sertifikasi Industri Hijau')"
                                class="group relative block w-full max-w-3xl cursor-pointer overflow-hidden rounded-2xl border border-slate-200 text-left">
                                <img src="{{ asset('storage/' . $spmLsih->image_path) }}"
                                    alt="Standar Pelayanan Minimal (SPM) Lembaga Sertifikasi Industri Hijau"
                                    class="h-auto w-full object-contain transition-transform duration-700 group-hover:scale-[1.03]">
                                <div
                                    class="absolute bottom-6 left-6 z-20 translate-y-4 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
                                    <span
                        ...
                        @else
                            <div class="w-full rounded-3xl border border-gray-100 bg-white p-24 text-center shadow-xl">
                                ...
                                <p class="font-medium text-slate-400">Gambar SPM Lembaga Sertifikasi Industri Hijau belum tersedia.</p>
                            </div>
                        @endif
                    </div>
                </section>
