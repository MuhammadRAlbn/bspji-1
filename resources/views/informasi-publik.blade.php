            ...
            <div class="max-w-6xl mx-auto px-6">
                <div class="grid md:grid-cols-2 gap-2 items-center">
                    <div class="pl-6">
                        ...
                        <div class="flex justify-start overflow-x-auto no-scrollbar gap-3 py-2 mt-4">
                            <button @click="tab = 'daftar'"
                                :class="tab === 'daftar' ? 'active-tab' : 'bg-slate-200/80 text-slate-700'"
                                class="tab-btn px-6 py-3 rounded-full text-md font-semibold whitespace-nowrap">
                    ...
                    <div class="flex justify-center md:justify-center">
                        <img src="{{ $heroImage }}" alt="Dokumen Informasi Publik" class="w-full max-w-md">
                    </div>
                </div>
            </div>
        </section>
        ...
        <section class="max-w-6xl mx-auto px-6 py-8">
            <div id="tab-content">
                <div id="content-daftar" x-show="tab === 'daftar'">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border px-2 border-slate-300 overflow-hidden mx-4">
                        @foreach ($daftarMenu as $menu)
                            <div @class([
                                'flex items-center justify-between p-3 md:px-10',
                                'border-b border-slate-50' => ! $loop->last,
                                'bg-slate-100' => in_array($loop->index, [1, 3], true),
                            ])>
                                <span class="font-semibold text-slate-700 text-lg">{{ $menu['label'] }}</span>
                    ...
                    <div class="mt-12 mx-4">
                        ...
                        <div class="bg-white rounded-2xl shadow-lg border border-slate-300 overflow-hidden h-[800px]">
                ...
                <div id="content-permohonan" x-show="tab === 'permohonan'" x-cloak>
                    ...
                    <div class="grid md:grid-cols-2 gap-8 items-start">
                        <div class="flex flex-col gap-6">
                            <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-400 flex items-center justify-between">
                                <div class="flex items-center gap-4">
                                    <div class="bg-orange-50 p-4 rounded-xl text-orange-500">
                                    ...
                                    <div>
                                        <h4 class="font-bold text-slate-800 text-lg">Form Permohonan Informasi</h4>
                                        <p class="text-slate-500 text-sm">File: {{ $permohonanNama ?? 'Belum tersedia' }}</p>
                                    </div>
                                </div>

                                @if ($permohonanPdf)
                                    <a href="{{ $permohonanPdf }}" target="_blank" rel="noopener noreferrer"
                                        class="bg-orange-500 text-white px-6 py-2.5 rounded-xl font-bold text-sm shadow-md shadow-orange-100 active:scale-95 transition-all">
                                        DOWNLOAD
                                    </a>
                                @else
                                    <span class="bg-slate-300 text-white px-6 py-2.5 rounded-xl font-bold text-sm cursor-not-allowed">
                                        DOWNLOAD
                                    </span>
                                @endif
                            </div>
                            ...
                            <div class="flex justify-center">
                                <img src="{{ $permohonanImage }}" alt="Permohonan Informasi" class="w-full max-w-sm h-auto">
                            </div>
                        </div>

                        <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-400">
                            <h4 class="font-bold text-slate-800 text-lg mb-6">Contoh formulir Permohonan Informasi</h4>
                            <div class="bg-slate-50 rounded-xl overflow-hidden border border-slate-100 flex items-center justify-center min-h-[400px]">
