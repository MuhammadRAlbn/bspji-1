<x-layouts.app title="Lembaga Pemeriksa Halal - BSPJI Banda Aceh" bodyClass="bg-slate-100/90">
<div x-data="{
    tab: 'ruang-lingkup',
    lightboxOpen: false,
    lightboxImage: '',
    ...
    <main class="mx-auto grid max-w-7xl grid-cols-1 items-start gap-8 px-4 sm:px-5 md:px-10 lg:grid-cols-[280px_1fr] lg:gap-[60px]">
        ...
        <article class="min-h-[85vh] pb-32 sm:pb-[450px]">
            ...
            <section x-show="tab === 'spm'" x-cloak x-transition:enter="transition ease-out duration-400" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
                <div class="flex flex-col justify-start gap-8">
                    @if($spmLph && $spmLph->image_path)
                        <button
                            type="button"
                            @click="openLightbox('{{ asset('storage/' . $spmLph->image_path) }}', 'Standar Pelayanan Minimal (SPM) Lembaga Pemeriksa Halal')"
                            class="group relative block w-full max-w-3xl cursor-pointer overflow-hidden rounded-2xl border border-slate-200 text-left"
                        >
                            <img
                                src="{{ asset('storage/' . $spmLph->image_path) }}"
                                alt="Standar Pelayanan Minimal (SPM) Lembaga Pemeriksa Halal"
                                class="h-auto w-full object-contain transition-transform duration-700 group-hover:scale-[1.03]"
                            >
                            <div class="absolute bottom-6 left-6 z-20 translate-y-4 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
                                <span class="rounded-full bg-slate-800 px-4 py-2 text-sm font-bold text-white shadow-sm">
                    ...
                    @else
                        <div class="w-full rounded-3xl border border-gray-100 bg-white p-24 text-center shadow-xl">
                            ...
                            <p class="font-medium text-slate-400">Gambar SPM Lembaga Pemeriksa Halal belum tersedia.</p>
                        </div>
                    @endif
                </div>
            </section>
