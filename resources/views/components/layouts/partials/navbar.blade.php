...
<nav
    ...
    <div
        ...
        @else
            class="mx-auto flex h-20 w-full max-w-6xl items-center gap-16 px-4 sm:px-6 lg:px-0"
        @endif>
        <div class="flex shrink-0 items-center pr-2">
            <a href="{{ url('/') }}"
                ...
                <img src="{{ asset('images/profil/logobalai.png') }}" alt="Logo BSPJI Banda Aceh"
                    @if ($isTransparent)
                        class="block h-[82px] w-auto max-w-none object-contain transition-all duration-300 sm:h-[92px]"
                    @else
        ...
        <div class="{{ $isTransparent ? 'ml-auto hidden min-w-0 shrink-0 items-center justify-end xl:flex' : 'hidden min-w-0 flex-1 items-center justify-center lg:flex' }}">
            <ul
                @if ($isTransparent)
                    :class="isSolid ? 'border-slate-200 bg-white/90 shadow-lg shadow-slate-950/10' : 'border-white/25 bg-neutral-950/60 shadow-2xl shadow-black/25'"
                                    ...
                                    @foreach ($menu['items'] as $item)
                                        <a href="{{ $item['url'] }}"
                                            @if ($isTransparent)
                                                ...
                                                class="block rounded-xl px-3 py-2 text-md font-medium transition"
                                            @else
    ...
    <div x-show="mobileOpen" x-transition:enter="transition ease-out duration-200"
        ...
        @if ($isTransparent)
            ...
            class="border-t px-4 py-5 backdrop-blur-xl xl:hidden sm:px-6"
        @else
            ...
            @foreach ($displayMenu as $menu)
                ...
                @else
                    <div
                        ...
                        <button type="button"
                            @if ($isTransparent)
                                :class="isSolid ? 'text-slate-700' : 'text-white/85'"
                        ...
                        <div x-show="mobileDropdown === '{{ $menu['key'] }}'"
                            @if ($isTransparent)
                                ...
                                class="space-y-1 border-t p-2"
                            @else
                                class="space-y-1 border-t border-slate-200 bg-slate-50/60 p-2"
                            @endif
                            style="display: none;">
                            @foreach ($menu['items'] as $item)
                                <a href="{{ $item['url'] }}"
